...
namespace CoinEx.Net.Clients.FuturesApi
{
    ...
    public class CoinExClientFuturesApi : RestApiClient, ICoinExClientFuturesApi, IFuturesClient
    {
        ...
        /// <inheritdoc />
        public ICoinExClientFuturesApiExchangeData ExchangeData { get; }

        /// <inheritdoc />
        public ICoinExClientFuturesApiTrading Trading { get; }

        internal CoinExClientFuturesApi(Log log, CoinExClient baseClient, CoinExClientOptions options)
            : base(log, options, options.FuturesApiOptions)
        {
            ...
            ExchangeData = new CoinExClientFuturesApiExchangeData(this);
            Trading = new CoinExClientFuturesApiTrading(this);
        ...
        protected override AuthenticationProvider CreateAuthenticationProvider(ApiCredentials credentials)
            => new CoinExAuthenticationProvider((CoinExApiCredentials)credentials);
        ...
        {
            var result = await SendRequestAsync<CoinExResult<object>>(uri, method, ct, parameters, signed, parameterPosition).ConfigureAwait(false);
            if (!result)
        ...
        {
            var result = await SendRequestAsync<CoinExResult<T>>(uri, method, ct, parameters, signed, parameterPosition, requestWeight: weight, ignoreRatelimit: ignoreRatelimit).ConfigureAwait(false);
            if (!result)
        ...
        /// <summary>
        /// Return the CoinEx trade symbol name from base and quote asset 
        /// </summary>
        ...
        {
            if (limit != null)
                throw new ArgumentException($"CoinEx doesn't support the {nameof(limit)} parameter for the method {nameof(IBaseRestClient.GetKlinesAsync)}", nameof(limit));
        ...
        async Task<WebCallResult<OrderId>> IFuturesClient.PlaceOrderAsync(string symbol, CommonOrderSide side, CommonOrderType type, decimal quantity, decimal? price, int? leverage, string? accountId, string? clientOrderId, CancellationToken ct)
        {
            if (!leverage.HasValue)
                throw new ArgumentException($"CoinEx required the {nameof(leverage)} parameter for {nameof(IFuturesClient.PlaceOrderAsync)}");
        ...
        private static FuturesKlineInterval GetKlineIntervalFromTimespan(TimeSpan timeSpan)
        {
            ...
            throw new ArgumentException("Unsupported timespan for CoinEx kline interval, check supported intervals using CoinEx.Net.Objects.CoinExKlineInterval");
        }
